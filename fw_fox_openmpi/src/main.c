...
int main(int argc, char **argv) {
    ...
    if(my_rank != P_MASTER_RANK) {
        ...
        // escrevendo matriz resultado no arquivo
        int i, len = strlen(argv[1]);
        for(i = len - 1; argv[1][i] != '/'; i--);
        i++;
        char *str_pm = "../util/mat_res/T(%d)_%s";
        char *caminho = malloc(64);
        sprintf(caminho, str_pm, num_procs, argv[1] + i);
        escreve_matriz(m, N, caminho);

        // escrevendo tempo de execução no arquivo
        char *str_pt = "../util/time_res/T(%d)_%s";
        sprintf(caminho, str_pt, num_procs, argv[1] + i);
        FILE *f = fopen(caminho, "aw");
        fprintf(f, "\n%.15lf", t_fim - t_ini);
        fclose(f);
